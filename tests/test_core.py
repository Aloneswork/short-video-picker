...
from public_browser import (
    ...
    _best_network_video,
    _browser_user_agent,
    _canonical_douyin_profile_url,
    _canonical_douyin_url,
    _canonical_douyin_work_url,
...
class CoreTests(unittest.TestCase):
    ...
    def test_frontend_uses_standard_thumbnail_ratios_and_marks_existing_media(self):
        ...
        self.assertIn("className='profile-block'", html)
        self.assertIn("主页作品 ${result.profile_index}/${result.profile_count}", html)
    ...
    def test_douyin_profile_and_work_urls_are_classified_and_canonicalised(self):
        profile = "https://www.douyin.com/user/MS4wLjABAAAAexample?from_tab_name=main"
        self.assertTrue(_is_douyin_profile_url(profile))
        self.assertEqual(
            _canonical_douyin_profile_url(profile),
            "https://www.douyin.com/user/MS4wLjABAAAAexample",
        )
        self.assertEqual(
            _canonical_douyin_work_url("https://www.douyin.com/video/7666677674693244646?from=profile"),
            "https://www.douyin.com/video/7666677674693244646",
        )
        self.assertEqual(_canonical_douyin_work_url("https://example.com/video/123"), "")

    def test_profile_response_entries_keep_order_and_build_work_urls(self):
        video = {
            "aweme_id": "7111111111111111111",
            "share_info": {
                "share_url": "https://www.douyin.com/video/7111111111111111111?previous_page=profile"
            },
        }
        note = {
            "aweme_id": "7222222222222222222",
            "images": [{"url_list": ["https://p3.douyinpic.com/a.webp"]}],
        }
        entries = _profile_aweme_entries(
            {
                "data": {
                    "aweme_list": [video, note],
                    "nested_duplicate": {"aweme_list": [video]},
                }
            }
        )
        self.assertEqual(entries, [video, note])
        self.assertEqual(
            [_profile_work_url(entry) for entry in entries],
            [
                "https://www.douyin.com/video/7111111111111111111",
                "https://www.douyin.com/note/7222222222222222222",
            ],
        )

    def test_profile_response_body_is_reused_without_constructing_an_api_request(self):
            ...
            "aweme_list": [
                {
                    ...
                    "video": {
                        "play_addr": {
                            "url_list": [
                                "https://v3.douyinvod.com/video/tos/example/?mime_type=video_mp4"
                            ]
                        }
                    },
                }
        ...
        class FakeClient:
                ...
                return [
                    {
                        ...
                        "params": {
                            "requestId": "profile-list-1",
                            "response": {
                                "url": "https://www.douyin.com/aweme/v1/web/aweme/post/?sec_user_id=test",
            ...
            def call(self, method, params=None, **kwargs):
                self.assert_get_body = (method, params)
                return {"body": __import__("json").dumps(response_payload)}

        client = FakeClient()
        pending = {}
        payloads = {}
        _collect_profile_response_payloads(client, pending, payloads)

        work_url = f"https://www.douyin.com/video/{work_id}"
        self.assertEqual(client.assert_get_body[0], "Network.getResponseBody")
        self.assertEqual(pending, {})
        self.assertEqual(payloads[work_url]["desc"], "公开主页作品")
    ...
    def test_public_browser_reads_at_most_thirty_profile_works(self):
        ...
        with patch("public_browser._free_port", return_value=54324):
        ...
        try:
            with patch("public_browser.time.sleep", return_value=None):
                result = browser.parse_douyin_profile(
    ...
    def test_public_browser_reads_profile_payload_cloned_by_page(self):
        ...
        class FakeClient:
            def __init__(self):
                self.capture_script_added = False

            def call(self, method, params=None, **kwargs):
                if method == "Page.addScriptToEvaluateOnNewDocument":
                    self.capture_script_added = True
                    return {}
                if method == "Runtime.evaluate":
                    page = {
                        "title": "响应用户的主页 - 抖音",
                        ...
                        "unavailable": "",
                        "challenge": False,
                        "profileResponses": [__import__("json").dumps(payload)],
                    }
                    return {"result": {"value": __import__("json").dumps(page)}}
                return {}
        ...
        client = FakeClient()
        with patch("public_browser._free_port", return_value=54325):
            browser = PublicBrowser()
        browser._page_client = lambda: client  # type: ignore[method-assign]
        try:
            with patch("public_browser.time.sleep", return_value=None):
                result = browser.parse_douyin_profile(
                    "https://www.douyin.com/user/profile-token",
    ...
    def test_profile_link_expands_works_and_marks_result_origin(self):
        instances = []
        profile_url = "https://www.douyin.com/user/profile-token"
        work_urls = [
            "https://www.douyin.com/video/7111111111111111111",
            "https://www.douyin.com/note/7222222222222222222",
        ]
        ...
        class FakeBrowser:
            ...
            def parse_douyin_profile(self, source_url, page_url="", limit=30):
                return ProfileResult(source_url, page_url, "测试主页", work_urls, ["主页读取完成"])

            def parse_douyin(self, source_url, page_url=""):
                item_id = extract_douyin_item_id(source_url)
                kind = "image" if "/note/" in source_url else "video"
                return ParseResult(
                    source_url,
                    page_url,
                    f"作品 {item_id}",
                    [MediaItem(source_url, page_url, f"https://cdn.example.com/{item_id}", kind)],
                )
        ...
        with (
            ...
            result = parser_assistant.parse_payload({"links": [profile_url]})

        self.assertTrue(result["ok"])
        self.assertEqual(len(result["results"]), 2)
        self.assertEqual([entry["origin_type"] for entry in result["results"]], ["profile", "profile"])
        self.assertEqual([entry["profile_index"] for entry in result["results"]], [1, 2])
        self.assertEqual({entry["profile_count"] for entry in result["results"]}, {2})
        self.assertEqual({entry["profile_title"] for entry in result["results"]}, {"测试主页"})
        self.assertIn("2 个作品来自 1 个用户主页", result["message"])
        fetch_url.assert_not_called()

    def test_profile_video_payload_avoids_reopening_each_work_page(self):
        profile_url = "https://www.douyin.com/user/profile-token"
        ...
        payload = {
            ...
            "video": {
                ...
                "cover": {
                    "url_list": [
                        "https://p3.douyinpic.com/example.webp?from=profile"
                    ]
                },
            },
        }
        ...
        class FakeBrowser:
            ...
            def parse_douyin_profile(self, source_url, page_url="", limit=30):
                return ProfileResult(
                    ...
                    [work_url],
                    [],
                    work_payloads={work_url: payload},
                )

            def parse_douyin(self, source_url, page_url=""):
                raise AssertionError("主页媒体数据完整时不应重复打开作品页")
        ...
        self.assertEqual(entry["title"], "主页列表视频")
        self.assertEqual(len(entry["items"]), 1)
        self.assertEqual(entry["items"][0]["kind"], "video")
        self.assertIn("douyinvod.com", entry["items"][0]["media_url"])
        self.assertIn("douyinpic.com", entry["items"][0]["preview_url"])
        self.assertIn("跳过重复打开", entry["debug"][1])

    def test_profile_expansion_respects_global_thirty_work_limit(self):
        profile_url = "https://www.douyin.com/user/profile-token"
        work_urls = [
            f"https://www.douyin.com/video/{7100000000000000000 + index}"
        ...
        class FakeBrowser:
            ...
            def parse_douyin_profile(self, source_url, page_url="", limit=30):
                return ProfileResult(source_url, page_url, "超量主页", work_urls, [])
            ...
            def parse_douyin(self, source_url, page_url=""):
                ...
                return ParseResult(
                    ...
                    page_url,
                    f"作品 {item_id}",
                    [MediaItem(source_url, page_url, f"https://cdn.example.com/{item_id}.mp4", "video")],
                )
        ...
        with (
            ...
            result = parser_assistant.parse_payload(
                {"links": [profile_url, "https://www.douyin.com/video/7999999999999999999"]}
            )

        self.assertTrue(result["ok"])
        self.assertEqual(len(result["results"]), 30)
        self.assertEqual(result["results"][-1]["profile_index"], 30)
        self.assertEqual({entry["profile_count"] for entry in result["results"]}, {30})
        self.assertIn("单批作品最多 30 个", result["message"])
        fetch_url.assert_not_called()

    def test_profile_failure_keeps_profile_origin_metadata(self):
        profile_url = "https://www.douyin.com/user/profile-token"
        ...
        entry = result["results"][0]
        self.assertEqual(entry["error_code"], "DY-PROFILE-NO-WORKS")
        self.assertEqual(entry["origin_type"], "profile")
        self.assertEqual(entry["profile_title"], "空主页")
        self.assertEqual(entry["profile_count"], 0)

    def test_profile_work_gets_one_final_same_session_retry(self):
        profile_url = "https://www.douyin.com/user/profile-token"
        work_url = "https://www.douyin.com/video/7111111111111111111"
        instances = []
        ...
        class FakeBrowser:
            ...
            def parse_douyin_profile(self, source_url, page_url="", limit=30):
                return ProfileResult(source_url, page_url, "重试主页", [work_url], [])

            def parse_douyin(self, source_url, page_url=""):
                self.work_calls += 1
                if len(instances) == 1 or self.work_calls == 1:
                ...
                return ParseResult(
                    ...
                    page_url,
                    "重试成功",
                    [MediaItem(source_url, page_url, "https://cdn.example.com/retry.mp4", "video")],
                )
        ...
        self.assertEqual(instances[0].work_calls, 1)
        self.assertEqual(instances[1].work_calls, 2)
        self.assertEqual(result["results"][0]["error_code"], "")
        self.assertIn("最后一次复查", result["results"][0]["debug"][1])
