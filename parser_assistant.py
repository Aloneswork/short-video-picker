...
from app_config import MAX_BATCH_LINKS
from core import (
    ParseResult,
...
def parse_payload(
    ...
    preview_jobs = []
    parsed_entries: list[ParseResult] = []
        ...
        progress_callback(
            {
                ...
                "total": len(links),
                "current": link,
                "results": [serialise(result) for result in parsed_entries[entry_start:]],
            }
        )
    ...
    def resolve_link(link: str) -> str:
        if _is_direct_douyin_work_url(link):
            return link
        try:
    ...
    def parse_work(link: str, page_url: str):
        nonlocal browser
        try:
            if extract_douyin_item_id(f"{link} {page_url}"):
                ...
                # A confirmed unavailable page can leave the anonymous session
    ...
    # Douyin's public pages can present a short-lived browser check to every
    # fresh session. Reuse one isolated Edge session for the whole batch so its
    # public-page cookies survive between links, and navigate serially rather
    # than opening several simultaneous anonymous pages that are more likely to
    # remain on the empty verification shell.
    try:
    ...
    results = [serialise(result) for result in parsed_entries]
